...
public:
    //开启会话触发的事件
    bool OnConnect(const MQTTProxy::Callback::MQTTClientSessionPtr&  session);

    //订阅事件
    bool OnSubscribe(const MQTTProxy::Callback::MQTTClientSessionPtr&  session, const MQTTProxy::MQTTSubscribe& subscribe);

    //取消订阅事件
    bool OnUnSubscribe(const MQTTProxy::Callback::MQTTClientSessionPtr&  session, const MQTTProxy::MQTTSubscribe& subscribe);
